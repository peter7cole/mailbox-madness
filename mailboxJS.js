...
mapText[0] =
  "An ice <strong>axe</strong> rides shotgun in the <strong>Outback</strong>";
mapText[1] = "Something glistens behind the <strong>gate</strong>";
mapText[2] =
  "<abbr title='Washington Trails Association'>WTA</abbr> " +
...
function keydownHandler(event) {
  if (event.keyCode === 13 && win === 0) {
    run();
  } else if (event.keyCode === 13 && win === 5) {
    endGame();
  } else if (event.keyCode === 13 && win > 0) {
    display();
  } else {
  }
}
  ...
  switch (item) {
    case "axe": // options for axe use
      if (here === 14 && axe[0] === 1) {
        axe[2] = 1;
        message =
          "Pulling out your axe, you're ready to climb safely to the <strong>east</strong>";
        mapText[14] =
          "A steep and exposed hillside heading <strong>east</strong>" +
    ...
    case "spikes": // options for spikes use
      if (here === 7 && spikes[0] === 1) {
        spikes[2] = 1;
        mapText[7] =
          "Your spikes are attached, ready to keep heading <strong>north</strong>!";
        message =
          "Attaching the spikes to your boots, you gain traction and may proceed";
      } else if (here !== 7 && spikes[0] === 1) {
        message = "Can't use the spikes here";
      } else if (spikes[0] === 0) {
        message = "You don't have the spikes in your pack";
      } else {
        console.log("error use spikes");
      }
      break;
    ...
    case "shovel": // options for shovel use
      if (here === 15 && shovel[0] === 1) {
        mail = 1;
        mapText[15] =
          "What a beautiful view, let's <strong>check</strong> the mail!";
        message =
          "You dug out the mailbox and can now <strong>check</strong> the mail!";
        images[15] = images[19];
  ...
  switch (item) {
    ...
    case "spikes": // options for take spikes
      if (spikes[0] === 0 && spikes[1] === here) {
        ...
        mapText[7] =
          "You should put your <strong>spikes</strong> on to get up this";
    ...
    case "shovel": // options for take shovel
      if (shovel[0] === 0 && shovel[1] === here && shovel[2] === 1) {
        ...
        shovel[0] = 1;
        mapText[10] =
          "Junction - North (Summit), West (New Trail), East (Old Trail)";
...
function checkF() {
  ...
  switch (item) {
    ...
    case "spikes": // options for check spikes
      if (spikes[0] === 1) {
        ...
        console.log("error check spikes");
      }
      break;
    ...
    case "snacks": // options for check snacks
      if (snacks[0] === 1 && snacks[2] === 0) {
        message =
          "Sour gummy worms, Rips, and an apple. DELICIOUS trail jet fuel.";
      } else if (snacks[0] === 0) {
        message = "You didn't pack any snacks yourself, rookie";
      } else if (snacks[2] === 1) {
        message = "You ate all the snacks already...";
      } else {
        console.log("error check snacks");
      }
      break;
    ...
    case "shovel": // options for check shovel
      if (shovel[0] === 1 || (here === 10 && shovel[2] === 1)) {
        ...
        console.log("error check shovel");
      }
      break;
    ...
    case "logs": // options for check logs
      if (here === 10 && shovel[0] === 0) {
        message =
          "A snow <strong>shovel</strong> is under the logs, you should take it";
        mapText[10] =
          "A three-way junction, a <strong>shovel</strong> lays by the sign";
        shovel[2] = 1;
        images[10] = images[16];
    ...
    case "outback": // options for check Subary Outback 2.5XT
      if (here === 0 && axe[0] === 0) {
        message =
          "There is an <strong>axe</strong> in the car, you should take it";
        ...
        console.log("error check outback");
      }
      break;
    ...
    case "sign": // options for check the various signs
      if (here === 2) {
        message =
          "New Trail: Longer than Old Trail, but less steep. " +
    ...
    case "mail": // options for check mail and ending the game
      if (here === 15 && mail === 1) {
        ...
        message =
          "You must dig out the mailbox with a <strong>shovel</strong> before checking the mail";
      } else if (here !== 15) {
        message = "You are not at the mailbox";
      } else {
        console.log("error check mail");
      }
      break;
  ...
  if (win === 0) {
    display();
  } // necessary to not skip first finish scene
  else {
  }
}
